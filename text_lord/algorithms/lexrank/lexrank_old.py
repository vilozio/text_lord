...
class LexRank(object):
    def __init__(self, docs_dir, ext='txt'):
        ...
        self.tfidf_scores = tfidf.get_TFIDF_scores(docs_dir, ext)
    ...
    def singleDocSummarization(self, doc_id, similarityFunct, normalizeFunct):
        ...
        self.matrix = self.buildSimilarityMatrix(self.num_of_sentences,
    ...
    def multiDocSummarization(self, similarityFunct, normalizeFunct):
        ...
        paths = []
        pathlist = Path(self.doc_dir).glob('**/*.txt')
        for path in pathlist:
            paths.append(str(path))
        num_of_docs = len(paths)
        for x in range(0, num_of_docs):
            curr_sentences = utils.getSentences(paths, x)
            curr_num_of_sentences = len(curr_sentences)
            sentences.append(curr_sentences)
            matrices.append(self.buildSimilarityMatrix(curr_num_of_sentences,
                                                       curr_sentences, x,
                                                       similarityFunct,
                                                       normalizeFunct))
    ...
    def buildSimilarityMatrix(self, num_of_sentences, sentences, doc_id,
                              similarityFunct, normalizeFunct):
        matrix = np.zeros((num_of_sentences, num_of_sentences))
        for x in range(0, num_of_sentences):
            vec_x = utils.mapTFIDFScores(sentences[x],
                                         self.tfidf_scores[doc_id])
            for y in range(0, num_of_sentences):
                if x == y:
                    matrix[x][y] = 1.0
                    continue
                vec_y = utils.mapTFIDFScores(sentences[y],
                                             self.tfidf_scores[doc_id])
    ...
    def rank(self, similarity_threshold, epsilon, iterations, continuous, damp,
             multi=False):
        if multi:
            return multiDocRank(self.matrix, self.sentences,
                                similarity_threshold, epsilon, iterations,
                                continuous, damp)
        else:
            return singleDocRank(self.matrix, self.sentences,
                                 similarity_threshold, epsilon, iterations,
                                 continuous, damp)


def multiDocRank(matrices, sentences, similarity_threshold, epsilon,
    ...
    for x in range(0, len(matrices)):
        ...
        transition_matrix = buildTransitionMatrix(similarity_matrix,
                                                  num_of_sentences,
                                                  similarity_threshold,
                                                  continuous)
        rank, score = powerIteration(transition_matrix, num_of_sentences,
                                     epsilon, iterations, damp)
        rankings.append(rank)
    return rankings, sentences


def singleDocRank(matrix, sentences, similarity_threshold, epsilon, iterations,
                  continuous, damp):
    num_of_sentences = len(matrix[0])
    similarity_matrix = reflectOverYX(matrix, num_of_sentences)
    transition_matrix = buildTransitionMatrix(similarity_matrix,
                                              num_of_sentences,
                                              similarity_threshold, continuous)

    rankings, score = powerIteration(transition_matrix, num_of_sentences,
                                     epsilon, iterations, damp)
    return rankings, sentences


def powerIteration(stochastic_matrix, num_of_sentences, epsilon, maxIterations,
                   damp):
    if damp > 0:
        curr_matrix = applyDampingFactor(stochastic_matrix, num_of_sentences,
                                         damp).transpose()
    else:
...
def buildTransitionMatrix(similarity_matrix, num_of_sentences,
